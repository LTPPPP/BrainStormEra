...
namespace BrainStormEra.Controllers.Lessons
{
    public class LessonController : Controller
    {
        ...
        // GET: View Lesson for Learner
        [HttpGet]
        public IActionResult ViewLessonLearner(string id)
        {
            // Lấy các bài học mà Learner có quyền truy cập (tùy logic của bạn)
            var lessons = _context.Lessons
                .Where(l => l.LessonTypeId == 1) // chỉ lấy các bài học video chẳng hạn
                .ToList();

            return View(lessons); // Trả về view dành riêng cho Learner
        }
    }
}
